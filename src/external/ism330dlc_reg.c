  ...
  if (ret == 0)
  {
    ...
    if (ret == 0)
    {
              ...
              (ctrl4_c.lpf1_sel_g << 3) + ctrl6_c.ftype)
      {
        ...
        case 0x08:
          *val = ISM330DLC_HP_DISABLE_LP1_STRONG;
          break;
...
int32_t platform_write(void *handle, uint8_t reg, const uint8_t *bufp, uint16_t len) {
    ism330dlcWrite write = { .read = ISM330_WRITE, .reg = (uint16_t)reg, .data = *bufp };
    ism330dlcRead  read;
...
int32_t platform_read(void *handle, uint8_t reg, uint8_t *bufp, uint16_t len) {
    for(int i = 0; i < len; i++) {
        ism330dlcWrite write = { .read = ISM330_READ, .reg = (uint16_t)(reg + i) };
        ism330dlcRead read;
