 ...
 *
 * @file
 * @brief Building Block USB HID device core functions
 *
 */
/*----------------------------------------------------------------------------*/
...
{
    USBD_CDC_HandleTypeDef  *hcdc;
    const PCD_HandleTypeDef *hpcd1 = pdev->pData;
    ...
    if ((pdev->ep_in[epnum].total_length > 0U) && ((pdev->ep_in[epnum].total_length % hpcd1->IN_ep[epnum].maxpacket) == 0U))
    {
